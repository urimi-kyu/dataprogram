...
def get_today_mdd_prediction(main_keyword):
    ...
    #3. [복구/추가] 8개 피처를 12개 피처로 패딩(Padding)하는 로직
    if len(disaster_vector_list) < mdd_predictor.INPUT_DIM:
        padding_count = mdd_predictor.INPUT_DIM - len(disaster_vector_list)
        # 부족한 개수만큼 0.0을 리스트에 추가 (12개로 맞춤)
        disaster_vector_list.extend([0.0] * padding_count)
